...
class Link_pred_Runner():
    ...
    def erun(self):
        ...
        # construct model
        d_real, discriminator, ae_model = get_model(model_str, placeholders, feas['num_features'], feas['num_nodes'], feas['features_nonzero'], cat = self.cat)
        ...
        for epoch in range(self.iteration):
            ...
            fh.write('Epoch=%04d, train_loss=%.5f, val_roc=%.5f, val_ap=%.5f' % (epoch + 1, avg_cost, val_roc_score[-1], ap_curr))
            fh.write('\r\n')
            if (epoch+1) % 10 == 0:
                ...
                fh.write('Test ROC score=%f, Test AP score=%f' % (roc_score, ap_score))
                fh.write('\r\n')
            fh.flush()
            fh.close()